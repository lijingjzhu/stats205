...
police_file <- "data/counted_2015.csv"
...
Looking at state, we see that with raw deaths, California takes the cake with over 150 police killings. Rounding out the top 5 we have California, Texas, Florida, Arizona, and Georgia. 
```{r}
police_killings %>%
  count(state) %>%
  top_n(10) %>%
...
Lastly, we turn the three date features (day, month, year) into a single date so it is easier to analyze. 
...
As an after thought, we also renamed the column `raceethnicity` to `race` so we can deal with shorter variable names. Yay. 
