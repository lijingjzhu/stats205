...
# Reading in data
guardian <- read_csv(guardian_file)[-1]
washington <- read_csv(washington_file)[-1]
...
education <- read_csv(education_file, col_names = FALSE)
income <- read_csv(income_file, col_names = FALSE)
housing <- read_csv(housing_file, col_names = FALSE)
```
...
Mapping shot and tasered to the same category
...
Getting rid of armed discrepencies
...
We also have some weird date variables going on. We realize in `The Counted`
dataset, we have a lot of missing dates of death as well. This makes the date
kind of wonky when we join the two together. 

```{r}
date_missing <- guardian %>% filter(is.na(date)) %>% .$name
df %>% filter(name.y %in% date_missing) %>% head()
```

```{r}
df <- df %>% filter(!(name.y %in% date_missing))
```
...
df <- df %>%
  ...
  select(-c(id, uid, name.y))
```

# Adding in income and education
...
# Data Filtering: Method of Death
